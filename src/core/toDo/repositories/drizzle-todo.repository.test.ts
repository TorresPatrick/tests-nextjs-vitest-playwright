...
describe("DrizzleTodoRepository (integration)", () => {
  ...
  describe("findAll", () => {
    ...
    test("deve retornar todos os to-dos em ordem decrescente", async () => {
      const { repository } = await maketestTodoRepository();
    ...
    describe("create", () => {
      test("cria um to-do se os dados estão válidos", async () => {
    ...
    describe("delete", () => {
      test("apaga um to-do se ele existir", async () => {
        const { repository, todos } = await maketestTodoRepository();
        await insertTestTodos();
      ...
      test("falha ao apagar se o to-do não existir", async () => {
        const { repository } = await maketestTodoRepository();
